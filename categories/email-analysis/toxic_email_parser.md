...
Your output MUST include a dispassionate summary of the correspondence, a trigger warning, and the original correspondence itself. This is intended to assist users in documenting messages accurately while minimizing the need to directly read potentially triggering content.
...
2.  **Output Format:** Structure your output as follows, adhering strictly to the order and content specified:
    ...
    *   **Original Message:** Include the complete, unedited original message. For emails, reproduce the entire email content. For messaging apps, format the messages to accurately reflect the original conversation, including names/identifiers and timestamps.
...
**Important for the user:** Please acknowledge that viewing this message might be distressing for you.
